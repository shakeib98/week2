...
describe('CustomTests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    expect(tornadoPoolBalance).to.be.equal((aliceDepositAmount - aliceWithdrawAmount).toString());
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // checking bobs new balance after sending to another account, event index 4 returns the required commitment.
    const filterNew = tornadoPool.filters.NewCommitment()
    const fromBlockNew = ethers.provider.getBlock()
    const eventsRemainingUtxo = await tornadoPool.queryFilter(filterNew, fromBlockNew.number)
    let bobRemainingUtxo
    bobRemainingUtxo = Utxo.decrypt(bobKeypair, eventsRemainingUtxo[4].args.encryptedOutput, eventsRemainingUtxo[4].args.index)

    expect(bobRemainingUtxo.amount).to.be.equal(bobSendAmount.sub(bobWithdrawAmount))


     // withdraws a part of his funds from the shielded pool
     const recipient = '0x8e7126dc2bA9E19d84c7d708F294A743FA4bcE63'
     const aliceWithdrawUtxo = new Utxo({amount: aliceDepositAmount.sub(bobSendAmount), keypair: aliceKeypair});
     await transaction({
       tornadoPool,
